...
public class Day07Part2 {
    ...
    static {
        ...
        RANK_LOOKUP.put("6", 8);
        RANK_LOOKUP.put("5", 9);
        RANK_LOOKUP.put("4", 10);
        RANK_LOOKUP.put("3", 11);
        RANK_LOOKUP.put("2", 12);
    }
    ...
    record Hand(String cards, int bid) implements Comparable<Hand> {
        ...
        HandType type() {
            ...
            if (characterCount.containsKey("J")) {
                Long jCount = characterCount.get("J");
                characterCount.put("J", 0L);
                String maxKey = Collections.max(characterCount.entrySet(), Map.Entry.comparingByValue()).getKey();
                characterCount.put(maxKey, characterCount.get(maxKey) + jCount);
            }
            ...
            if (characterCount.containsValue(5L)) {
                ...
                long value = characterCount.values()
                        .stream()
                        .filter(v -> v != 0)
                        .reduce(1L, (sub, elem) -> sub * elem);
                if (value == 4) {
